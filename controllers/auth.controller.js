...
const register = async (req, res) => {
  try {
    ...
    const { full_name, username, email, password, phone, city, state } = req.body;
...
const login = async (req, res) => {
  try {
    ...
    if (!credential || !password) {
      return apiResponse.error(res, 'Email or username and password required', 400);
    }

    let user =
      (await User.scope('withPassword').findOne({ where: { email: credential } })) ||
      (await User.scope('withPassword').findOne({ where: { username: credential } }));
...
const refresh = async (req, res) => {
  try {
    ...
    if (!refreshToken) return apiResponse.error(res, 'Refresh token missing', 400);

    const row = await RefreshToken.findOne({
      where: { token: refreshToken },
      include: [{ model: User, attributes: ['id', 'username', 'email', 'role', 'is_banned'] }],
    });
